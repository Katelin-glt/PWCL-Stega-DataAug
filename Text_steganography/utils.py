...
class Vocabulary(object):
    ...
    def _build_vocabulary(self):
        ...
        word_distribution = sorted(collections.Counter(words_all).items(),
        ...
        start_word_distribution = sorted(
                collections.Counter(start_words).items(), 
                key=lambda x: x[1], 
                reverse=True)
    ...
    def _build_corpus(self):
        ...
        for data_path in self._data_path:
            ...
            for sentence in sentences:
                ...
                if (len(sentence) >= self._min_len) and \
                                (len(sentence) <= self._max_len):
...
def split_corpus(data_path, train_path, test_path, max_len=200, min_len=5, 
        ratio=0.8, seed=0, encoding='utf8'):
...
class Generator(object):
    ...
    def _padding(self, batch_data):
        ...
        for i in range(len(batch_data)):
            batch_data[i] += [self._vocabulary.w2i['_PAD']] * \
                            (max_length - len(batch_data[i]))
        return np.array(list(batch_data))


    def _build_generator(self, batch_size, shuffle=True, padding=True):
